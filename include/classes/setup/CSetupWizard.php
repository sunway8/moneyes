...
class CSetupWizard extends CForm {
	...
	function __construct() {
		...
		$this->stage = [
			...
			3 => [
				'title' => _('Moneyes server details'),
				'fnc' => 'stage3'
			],
			4 => [
	...
	function stage0() {
		$setup_title = (new CDiv([new CSpan(_('Welcome to')), 'Moneyes 3.0']))->addClass(ZBX_STYLE_SETUP_TITLE);
	...
	function stage2() {
		...
		switch ($DB['TYPE']) {
			case ZBX_DB_SQLITE3:
				$table->addRow(
					_('Database file'),
					(new CTextBox('database', $this->getConfig('DB_DATABASE', 'Moneyes')))
			...
			default:
				...
				$table->addRow(_('User'),
					(new CTextBox('user', $this->getConfig('DB_USER', 'moneyes')))->setWidth(ZBX_TEXTAREA_SMALL_WIDTH)
				);
	...
	function stage3() {
		...
		return [
			new CTag('h1', true, _('Moneyes server details')),
			(new CDiv([
				new CTag('p', true, _('Please enter the host name or host IP address and port number of the Moneyes server, as well as the name of the installation (optional).')),
				$table
			]))->addClass(ZBX_STYLE_SETUP_RIGHT_BODY)
		];
	}
	...
	function stage4() {
		...
		$table->addRow((new CSpan(_('Moneyes server')))->addClass(ZBX_STYLE_GREY), $this->getConfig('ZBX_SERVER'));
		$table->addRow((new CSpan(_('Moneyes server port')))->addClass(ZBX_STYLE_GREY), $this->getConfig('ZBX_SERVER_PORT'));
		$table->addRow((new CSpan(_('Moneyes server name')))->addClass(ZBX_STYLE_GREY), $this->getConfig('ZBX_SERVER_NAME'));
	...
	function stage5() {
		...
		if ($error) {
			...
			$message_box = null;
			$message = [
				(new CTag('h1', true, _('Congratulations! You have successfully installed Moneyes frontend.')))
	...
	function checkConnection() {
		...
		$DB['DATABASE'] = $this->getConfig('DB_DATABASE', 'Moneyes');
		...
		if (!$result = DBconnect($error)) {
			...
			if ($result) {
				$result = DBexecute('CREATE TABLE Moneyes_installation_test (test_row INTEGER)');
				$result &= DBexecute('DROP TABLE Moneyes_installation_test');
			}
		}
	...
	function eventHandler() {
		...
		elseif ($this->getStep() == 2) {
			...
			$this->setConfig('DB_DATABASE', getRequest('database', $this->getConfig('DB_DATABASE', 'Moneyes')));
